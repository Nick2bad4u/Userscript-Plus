...
`;
  ...
  const getUAData = () => {
    ...
    try {
      ...
      userjs.isMobile = /Mobile|Tablet/.test(navigator.userAgent ?? "") ||
        mobile ||
      ...
      const Timeout = class {
        ...
        set(delay, reason) {
          return new Promise((resolve, reject) => {
            const id = setTimeout(() => {
              Object.is(reason, null) || Object.is(reason, undefined)
                ? resolve()
                : reject(reason);
              this.clear(id);
            }, delay);
      ...
      class Tabs {
        ...
        hasTab(...params) {
          for (const p of params) {
            ...
            const content = normalizeTarget(this.Tab.get(p)).filter((t) =>
              p === t.dataset.host
            );
            if (isBlank(content)) {
              return false;
    ...
    makeError(...ex) {
      ...
      for (const e of ex) {
        str += `${
          typeof e === "string"
            ? e
            : `${e.cause ? `[${e.cause}] ` : ""}${e.message} ${e.stack ?? ""}`
        }\n`;
        if (isObj(e)) {
          if (e.notify) {
    ...
    try {
      ...
      const createjs = (ujs, engine) => {
        ...
        const fcreated = make("mu-js", "mujs-list", {
          textContent: `${i18n$("created_date")}: ${
            language.toDate(ujs.created_at)
          }`,
        });
        ...
        const ftotal = make("mu-js", "mujs-list", {
          textContent: `${i18n$("total_installs")}: ${
        ...
        if (engine) {
          ...
          // }
          if (
            !engine.includes("fork") && cfg.recommend.others &&
          ...
          try {
            ...
            for (const engine of this.engines) {
                  ...
                  _mujs: {
                    ...
                    code: {
                      ...
                      request: async function (translate = false, code_url) {
                        ...
                        const applies_to_names = code_obj
                          .calculate_applies_to_names();
              ...
              const toQuery = (fallback) => {
                if (engine.query) {
                  return decodeURIComponent(engine.query).replace(
                    /\{host\}/g,
                    host,
                  );
                }
                    ...
                    for (const i of qsA(".col-sm-8 .tr-link", selected)) {
                      ...
                      const ujs = mergeTemplate(
                        _mujs({
                          ...
                          url: fixurl,
                          code_url: `${
                            fixurl.replace(/\/scripts/gi, "/install")
                          }.user.js`,
                          total_installs: dom.text(qs("td:nth-child(2) p", i)),
                          created_at: dom.attr(
                            qs("td:nth-child(4) time", i),
                            "datetime",
                          ),
      ...
      const makecfg = () => {
          ...
          if (type === "checkbox") {
            ...
            if (/(greasy|sleazy)fork|openuserjs|gi(thub|st)/gi.test(nm)) {
              for (const engine of cfg.engines) {
                ...
                if (engine.query) {
                  const d = DEFAULT_CONFIG.engines.find((e) =>
                    e.name === engine.name
                  );
                    ...
                    onchange(evt) {
                      ...
                      try {
                        engine.query = encodeURIComponent(
                          new URL(evt.target.value).toString(),
                        );
                      } catch (ex) {
                        err(ex);
                      }
        ...
        for (
          ...
          const nm = inp.dataset.name === "recommend-author"
            ? "author"
            : "others";
